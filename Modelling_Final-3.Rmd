...
author: "Shawal Fida, Bryson Burr, Mike Hanger"
date: "March 4, 2025"
output:
  html_document:
...
To hopefully create a more accurate model, we plan on creating multiple new columns
from the information we have been given. Those new columns include: 

- Credit to income ratio 
- Debt to income ratio
- Credit to goods price ratio
- Income per family member
- Age in years rather than days
- Employment duration in years rather than days
- Employment to age ratio
- Document count (summing flags for document presence)
- Contact information count (summing all different contact methods)
- External source score average
- External source score buckets 
- Age groups
- Income groups
- Credit amount groups

Included in this is creating a new column called Days_Employed_Anom that puts a 1
for anytime there was a placeholder value of 365243 in the Days Employed column

## Handling of NA's

To prepare our datasets for modeling, we will first find any columns that have over
60% of the data missing and get rid of them, as they will not provide enough
information to be useful for the models. For the rest of the missing values, we plan
on imputing them using the median. The median was chosen as its easy to copute, 
understand, and preserves the distribution shape. The placeholder values in
Days Employed will be replaced by NA's and then imputed with the median, with
a new column to indicate which rows had a placeholder value in that column. 

## Overall Data Cleaning

To finish preparing our data for modeling, we will convert all categorical variables
to factors, as it will help our models handle the information better than the raw
categorical information. For any outliers we find, we will create a column similar 
to that of Days_Employed_Anom that puts a 1 for that row where it has an outlier
in certain columns. Lastly, any categorical columns with multiple values separated
by a comma will be replaced with "and" for the models to be able to process that
information.
...
To further explain, the confusion matrices show that the model still defaults toward
the positive classifier under the 0.5 threshold. This means that there is still an
abundance of false positives. Once again, when we change the threshold to 0.1, we
get overall improved balances. From the original version of this model to this final
product, we see an improvement in the balanced accuracy from 66.62% to 66.66%, which
...
We are going to try creating a decision tree with weights too try and help
...
models performed better than the decision trees, but did not improve upon
...
We decided to use the first logistic regression model to submit to Kaggle, as that
performed the best with the AUC, computing time, and is easily understandable. The run 
time was quick as it took only a few seconds to train. You can also see that the overall
results of this model are more accurate in predicting who will not have payment difficulties,
but was more geared toward that prediction and provided a large number of false positives.
Through modifying the threshold values, we can see that the number of false positives
can be reduced with a lower value as opposed to a higher value. The highest balanced
accuracy result is with the random forest method using a threshold value of 0.1,
attaining a result of 66.96%.
Overall, the model had a training AUC of .7246, and a validation AUC of .7339. 
We got a Kaggle score of .6978, putting usat about 6200 on the Kaggle leaderboard. 

# Results

From this process we have learned that we have a lot more work to do to create a
model that better answers the business question. As of right now, while we were
able to beat the benchmark random forest model on Kaggle, we don't believe we are
accurately able to answer the business question. We tried multiple techniques
such as feature engineering, hyperparameter tuning, and feature selection to improve
the models performance. However, even after using those techniques, the improvement
in AUC was either very marginal or non-existent. This suggests to us that we need
to try other methods to improve our models. That could mean better feature engineering
or selection, using resampling methods to improve the class imbalance, or simply choosing 
different models, such as an emsemble method, to make our predictions. 
...
Contribution: Worked on the introduction, data cleaning, exploratory data analysis (EDA), feature engineering, and preparation of training and validation sets. Established baseline performance metrics, fitted a logistic regression model, and added interaction terms to improve model performance.
...
Contribution: Assisted with data cleaning in initial EDA. Fitted all data models with confusion matrices and modifying 
threshold parameters. Added notes regarding confusion matrices outcomes/interpretation.
